	...
	for batchStart := 0; batchStart < len(structs); batchStart += opts.BatchSize {
		...
		for i, item := range batch {
			...
			go func(itemIndex int, structItem any) {
				...
				// Process the struct
				err := c.ProcessStruct(batchCtx, structItem)
...
func (c *Crypto) DecryptStructsBatch(ctx context.Context, structs []any, options ...*BatchProcessOptions) (*BatchProcessResult, error) {
	...
	for i, item := range structs {
		...
		go func(itemIndex int, structItem any) {
			...
			// Decrypt the struct
			err := c.DecryptStruct(batchCtx, structItem)
...
// GetProcessingStats returns performance statistics for the crypto instance
