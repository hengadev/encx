...
func TestHashSecure(t *testing.T) {
	...
	t.Run("empty pepper should error", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, encx.ErrUninitializedPepper)
	})
...
// isZeroPepper checks if pepper is empty or contains only zero bytes
...
func createTestCrypto(t *testing.T) *encx.Crypto {
	t.Helper()
	return &encx.Crypto{
		pepper:       []byte("test-pepper-1234567890"),
		argon2Params: encx.DefaultArgon2Params,
	}
}
