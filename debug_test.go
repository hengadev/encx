...
// Simple debug test to understand what's going wrong
func TestDebugCrypto(t *testing.T) {
	...
	// Test struct with all required fields
	type SimpleStruct struct {
		PlainField      string `json:"plain_field"`
		EncryptField    string `encx:"encrypt" json:"encrypt_field"`
		EncryptFieldEncrypted []byte `json:"encrypt_field_encrypted"` // Required target field
		DEK             []byte `json:"-"`
	...
	assert.Equal(t, "secret-value", testStruct.EncryptField)
}
