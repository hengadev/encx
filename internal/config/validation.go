...
func (v *Validator) ValidateConfig(config *Config) error {
	return v.ValidateConfigForEnvironment(config)
}
...
func (v *Validator) ValidateConfigForEnvironment(config *Config) error {
	...
	// Validate pepper (auto-generated, just check it's not empty)
	if err := v.validateAutoGeneratedPepper(config.Pepper); err != nil {
		return fmt.Errorf("pepper validation failed: %w", err)
	}
...
// validateAutoGeneratedPepper validates auto-generated pepper
func (v *Validator) validateAutoGeneratedPepper(pepper []byte) error {
	if len(pepper) == 0 {
